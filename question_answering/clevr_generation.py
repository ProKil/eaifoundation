...
parser.add_argument(
    "--forbid_key_value_json",
    type=str,
    default=None,
    help="JSON file with forbidden key-value pairs",
)
...
def precompute_filter_options(scene_struct, metadata):
    ...
    elif metadata["dataset"] == "DATASETNAME-v1.0":
        attr_keys = ["color", "material", "objecttype"]
    else:
        assert False, "Unknown dataset %s" % metadata["dataset"]
...
def add_empty_filter_options(attribute_map, metadata, num_to_add):
    ...
    if metadata["dataset"] == "CLEVR-v1.0":
        ...
        attr_keys = ["ObjectType", "Color", "Material"]
    else:
        assert False, "Unknown dataset %s" % metadata["dataset"]
...
def other_heuristic(text, param_vals):
    ...
    for k1, k2 in key_pairs:
        ...
        if v1 != "" and v2 != "" and v1 != v2:
            print(
                "other has got to go! %s = %s but %s = %s" % (k1, v1, k2, v2)
            )
            remove_other = True
            break
    if remove_other:
...
def instantiate_templates_dfs(
    ...
    answer_counts,
    synonyms,
    forbid_key_value: Optional[Dict[str, List[str]]] = None,
    max_instances=None,
    verbose=False,
):
    ...
    while states:
        ...
        if state["next_template_node"] == len(template["nodes"]):
            ...
            if (
                ...
                and len(final_states) == max_instances
            ):
                break
            continue
        ...
        if next_node["type"] in special_nodes:
            ...
            for k in filter_option_keys:
                ...
                for param_name, param_val in zip(filter_side_inputs, k):
                    param_type = param_name_to_type[param_name]
                    if (
                        forbid_key_value is not None
                        and param_val in forbid_key_value[param_type]
                    ):
                        skip = True
                        break
                    ...
                    if param_val is not None:
                        ...
                        if (
                            ...
                            metadata["dataset"] == "DATASETNAME-v1.0"
                            and param_type == "ObjectType"
                        ):
                            param_val = "thing"
                        else:
                            param_val = ""
...
def main(args):
        ...
        for key, template in templates.items():
            ...
            if final_dtype == "Integer":
                if metadata["dataset"] == "CLEVR-v1.0":
                    ...
                    answers = list(range(0, 21))  # not tested
    ...
    for i, scene in enumerate(all_scenes):
        ...
        scene_struct = scene
        print(
            "starting image %s (%d / %d)" % (scene_fn, i + 1, len(all_scenes))
        )
