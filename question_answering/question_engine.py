...
def vg_relate_handler(scene_struct, inputs, side_inputs):
    ...
    for rel in scene_struct["relationships"]:
        if (
            rel["predicate"] == side_inputs[0]
            and rel["subject_idx"] == inputs[0]
        ):
...
execute_handlers = {
    ...
    "filter_objecttype": make_filter_handler("objecttype"),
    ...
    # 'query_objecttype': make_query_handler('objecttype'),
    "exist": exist_handler,
...
def is_degenerate(
    question, metadata, scene_struct, answer=None, verbose=False
):
    """
