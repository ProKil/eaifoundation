...
def teleport_to(ctlr, state):
    ...
    objects = {
        n: [int(m[0]), int(m[1]), int(m[2] - m[0]), int(m[3] - m[1])]
        for n, m in e.instance_detections2D.items()
    }
    detections = {}
...
def find_shortes_terminal_path(state, table, depth=0, max_depth=4):
    ...
    for action in actions:
        ...
        new_depth = find_shortes_terminal_path(
            new_state, table, depth=depth + 1, max_depth=max_depth
        )
