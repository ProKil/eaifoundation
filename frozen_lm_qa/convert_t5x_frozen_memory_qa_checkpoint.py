...
from frozen_lm_qa.flax_model import (
...
def convert_t5x_checkpoint_to_flax(
    ...
    config = T5ConfigWithMemory.from_pretrained(config_name)
    flax_model = FlaxT5ForMemoryQuestionAnswering(config=config)
    ...
    for layer_index in range(config.num_layers):
        ...
        if split_mlp_wi:
            flax_model.params["encoder"]["block"][str(layer_index)]["layer"][
                "1"
            ]["DenseReluDense"]["wi_0"]["kernel"] = t5x_mlp_wi_0
            flax_model.params["encoder"]["block"][str(layer_index)]["layer"][
                "1"
            ]["DenseReluDense"]["wi_1"]["kernel"] = t5x_mlp_wi_1
        else:
            flax_model.params["encoder"]["block"][str(layer_index)]["layer"][
                "1"
            ]["DenseReluDense"]["wi"]["kernel"] = t5x_mlp_wi
    ...
    # prompt_generator
    flax_model.params["prompt_generator"] = t5x_model["target"][
        "prompt_generator"
    ]
